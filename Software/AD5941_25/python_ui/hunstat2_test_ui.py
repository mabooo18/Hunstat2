...
class TestUI(tk.Tk):
    ...
    def _generate_dummy_for_method(self, method):
        ...
        if method == "CV":
            ...
            start_v = self._coerce_to_volt(float(self.dpv_start_var.get()))
            end_v = self._coerce_to_volt(float(self.dpv_end_var.get()))
            step_v = max(self._coerce_to_volt(float(self.dpv_step_var.get())), 1e-6)
            pulse_amp = max(0.001, self._coerce_to_volt(float(self.dpv_amp_var.get())))
            total = min(2500, max(10, int(abs(end_v - start_v) / step_v) + 1))
            direction = 1 if end_v >= start_v else -1
